...
json에서 데이터를 받아올 때 `item['tags']`는 `dynamic 타입`이기 때문에 String 타입으로 변환해야 한다. 변환하는 방법은 다음과 같다.
```dart
RestaurantCard(
...
```
:::note
[Code Factory 중급](https://github.com/rookedsysc/Flutter-Study/blob/main/Project/lv2_actual/lib/restaurant/view/restaurant_screen.dart)
:::
